...
"""
    ...
    def _get_kserve_version(self) -> str:
        try:
            result = self._run_command([
                "kubectl", "get", "deployment", "kserve-controller-manager", 
                "-n", "kserve-system", "-o", "jsonpath={.spec.template.spec.containers[0].image}"
            ])
    ...
    def _get_node_info(self, gpu: Dict) -> str:
        try:
            selector = ",".join([f"{k}={v}" for k, v in gpu.get('node_selector', {}).items()])
            result = self._run_command([
                "kubectl", "get", "nodes", f"--selector={selector}", 
    ...
    def run_single_benchmark(self, gpu: Dict, model: Dict, traffic: Dict) -> Optional[Path]:
        """Run a single benchmark configuration"""
        run_id = self._generate_run_id(gpu['name'], model['name'], traffic['name'])
        run_dir = self.output_dir / run_id
        
        logger.info(f"Starting benchmark: {gpu['name']} + {model['name']} + {traffic['name']}")
        ...
        try:
            ...
            deploy_cmd = [
                "kvmini", "deploy",
                "--namespace", "benchmark",
                "--service", f"{model['name']}-ref", 
                "--model-uri", model['uri'],
                "--runtime", "vllm",
                "--gpu-limit", str(gpu['gpu_limit'])
            ]
            ...
            bench_cmd = [
                ...
                "--service", f"{model['name']}-ref",
                "--model", model['name'],
                "--requests", str(traffic['requests']),
                "--concurrency", str(traffic['concurrency']),
                "--max-tokens", str(traffic.get('max_tokens', 64)),
                "--bundle", 
                "--run-id", run_id
            ]
            ...
            try:
                self._run_command([
                    "kubectl", "delete", "inferenceservice", f"{model['name']}-ref", 
                    "-n", "benchmark", "--ignore-not-found"
                ])
            except:
                pass
            return None
    
    def _validate_results(self, results: Dict, gpu: Dict, model: Dict, traffic: Dict) -> bool:
        ...
        if error_rate > thresholds['error_rate_max']:
            logger.error(f"Error rate {error_rate} exceeds {thresholds['error_rate_max']}")
            return False
        
        # Check minimum throughput
        throughput = results.get('throughput_rps', 0)
        if throughput < thresholds['min_throughput_rps']:
            logger.error(f"Throughput {throughput} below {thresholds['min_throughput_rps']} RPS")
            return False
        ...
        if warm_p95 > 0 and (cold_p95 / warm_p95) > thresholds['cold_multiplier_max']:
            logger.error(f"Cold multiplier {cold_p95/warm_p95} exceeds {thresholds['cold_multiplier_max']}")
            return False
    ...
    def run_matrix(self, gpu_filter: Optional[str] = None, 
                   model_filter: Optional[str] = None) -> List[Path]:
        ...
        logger.info(f"Reference matrix completed: {len(successful_runs)}/{total_runs} successful")
    ...
    def _generate_summary(self, runs: List[Path]) -> None:
        ...
        logger.info(f"Matrix summary written to {self.output_dir / 'matrix_summary.json'}")


def main():
    parser = argparse.ArgumentParser(description="Reference runs matrix executor")
    parser.add_argument("--config", default="reference-matrix.yaml", 
                       help="Matrix configuration file")
    parser.add_argument("--output-dir", default="artifacts/reference",
