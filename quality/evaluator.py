...
class QualityEvaluator:
    ...
    def __init__(self, model_endpoint: str, model_name: str, 
                 tasks: Optional[List[str]] = None, num_samples: int = 100):
        ...
        samples = [
            {
                ...
                "choices": ["gives the dog a treat to calm it down", "runs after the dog with a hose", "ignores the dog and fills the bucket", "gets more upset with the dog"],
                "correct": 1
            },
            {
                "context": "A man is in the garage working on his car. He opens the hood and checks the oil. Then he...", 
                "choices": ["closes the hood and goes inside", "adds more oil to the engine", "starts the car to test it", "cleans his hands with a rag"],
                "correct": 1  
            },
            {
                "context": "The kids are at a birthday party. They're sitting around a table with a cake. Someone...",
                "choices": ["blows out the candles on the cake", "cuts the cake into pieces", "sings happy birthday song", "takes photos of everyone"],
                "correct": 0
            }
        ]
...
class ParetoAnalyzer:
    ...
    def classify_pareto_bucket(results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, (q1, l1, c1, idx1) in enumerate(points):
            ...
            for j, (q2, l2, c2, idx2) in enumerate(points):
                if i != j:
                    ...
                    if q2 <= q1 and l2 <= l1 and c2 <= c1 and (q2 < q1 or l2 < l1 or c2 < c1):
                        is_dominated = True
                        break
        ...
        for i, result in enumerate(results):
            ...
            enhanced["pareto_bucket"] = "on-pareto" if i in pareto_indices else "off-pareto"
            enhanced_results.append(enhanced)
        
        logger.info(f"Pareto analysis: {len(pareto_indices)}/{len(results)} configurations on frontier")
        return enhanced_results


def integrate_quality_eval(results_file: str, model_endpoint: str, model_name: str) -> None:
    ...
    logger.info(f"✅ Quality evaluation integrated. Score: {quality_results['quality_score']:.1f}")
...
def main():
    ...
    parser.add_argument("--tasks", nargs="+", default=QualityEvaluator.DEFAULT_TASKS,
    ...
    if args.results_file:
        ...
        # Standalone mode
        evaluator = QualityEvaluator(args.endpoint, args.model, args.tasks, args.samples)
