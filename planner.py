...
class CapacityPlanner:
    ...
    def _calculate_base_capacity(self, target_rps: float, p95_budget_ms: float, 
                                mix_profile: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _calculate_costs(self, capacity: Dict[str, Any], region: str = "us-central1") -> Dict[str, Any]:
        """Calculate monthly costs for capacity options"""
        
        region_multiplier = self.cost_config.get("regions", {}).get(region, {}).get("multiplier", 1.0)
        ...
        for option in capacity["options"]:
            ...
            # CPU costs
            cpu_monthly = (self.cost_config["cpu_per_hour"] * cpu_cores * 
                          hours_per_month * region_multiplier)
            
            # Memory costs  
            memory_monthly = (self.cost_config["memory_per_gb_hour"] * memory_gb * 
                            hours_per_month * region_multiplier)
            
            # Storage costs (assume 100GB persistent disk per GPU)
            storage_gb = gpu_count * 100
            storage_monthly = (self.cost_config["storage_per_gb_hour"] * storage_gb * 
                             hours_per_month * region_multiplier)
    ...
    def _calculate_warm_pool_sizing(self, target_rps: float, mix_profile: Dict[str, Any]) -> Dict[str, Any]:
        """Calculate cold-start warm pool requirements"""
        
        # Cold start characteristics
        cold_start_time_s = mix_profile.get("cold_start_time_s", 45)
        cold_start_frequency = mix_profile.get("cold_start_frequency", 0.1)  # 10% of requests
        target_cold_time_s = mix_profile.get("target_cold_time_s", 10)  # Target cold time
        
        # Calculate warm pool size to minimize cold starts
        expected_cold_requests_per_min = target_rps * 60 * cold_start_frequency
        warm_pool_size = math.ceil(expected_cold_requests_per_min * (cold_start_time_s / 60))
    ...
    def plan_capacity(self, target_rps: float, p95_budget_ms: float, 
                     mix_profile: Dict[str, Any], region: str = "us-central1",
                     run_history: Optional[List[str]] = None) -> Dict[str, Any]:
        """Generate complete capacity and budget plan"""
        
        # Load historical data if provided
        historical_runs = []
        if run_history:
            historical_runs = self._load_run_history(run_history)
        ...
        recommendations = self._generate_recommendations(capacity, costs, warm_pool, 
                                                       target_rps, p95_budget_ms)
    ...
    def _generate_recommendations(self, capacity: Dict[str, Any], costs: Dict[str, Any],
                                ...
                                p95_budget_ms: float) -> List[Dict[str, Any]]:
        ...
        for i, option in enumerate(capacity["options"]):
            ...
            gpu_utilization = target_rps / (option["gpu_count"] * option["effective_rps"])
            
            # Score recommendation (lower is better)
            cost_score = cost_data["costs"]["total_monthly"] / 1000  # Normalize to ~1-10 range
            ...
            recommendations.append({
                ...
                "rationale": self._get_rationale(option, cost_data, gpu_utilization)
            })
    ...
    def _get_rationale(self, option: Dict[str, Any], cost_data: Dict[str, Any],
    ...
    def generate_report(self, plan: Dict[str, Any], output_file: Optional[str] = None) -> str:
...
def main():
    ...
    parser.add_argument("--p95-budget", type=float, required=True,
                       help="P95 latency budget in milliseconds")
    parser.add_argument("--mix", default="profile.yaml", 
                       help="Traffic mix profile YAML")
    parser.add_argument("--region", default="us-central1",
                       help="Cloud region for pricing")
    parser.add_argument("--cost-file", default="cost.yaml",
                       help="Cost configuration file")
    ...
    report = planner.generate_report(plan, report_file)
