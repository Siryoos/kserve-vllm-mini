...
class QuantizationSweepRunner:
    ...
    def _run_benchmark(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        env.update(
            {
                ...
                "VLLM_GPU_MEMORY_UTILIZATION": str(
                    config.get("gpu_memory_utilization", 0.9)
                ),
                "VLLM_ENFORCE_EAGER": (
                    "true" if config.get("enforce_eager", False) else "false"
                ),
            }
        )
        ...
        bench_cmd = [
            ...
            "bench",
            "--namespace",
            self.config["benchmark"]["namespace"],
            "--service",
            f"{self.config['benchmark']['service']}-{config['name']}",
            "--model",
            self.config["benchmark"]["model"],
            "--requests",
            str(self.config["benchmark"]["requests"]),
            "--concurrency",
            str(self.config["benchmark"]["concurrency"]),
            "--max-tokens",
            str(config.get("max_tokens", 64)),
            "--run-id",
            run_id,
        ]
        ...
        try:
            ...
            result = subprocess.run(
                bench_cmd, capture_output=True, text=True, env=env, timeout=600
            )
            ...
            if result.returncode == 0:
                ...
                if results_file.exists():
                    ...
                    if self.config.get("quality_eval", {}).get("enabled", False):
                        benchmark_results = self._add_quality_eval(
...
"""
        ...
        try:
            ...
            if endpoint_result.returncode == 0 and endpoint_result.stdout:
                ...
                quality_cmd = [
                    ...
                    "math",
                    "--samples",
                    "10",  # Small sample for speed
                ]

                quality_result = subprocess.run(
                    quality_cmd, capture_output=True, text=True, timeout=300
                )
        ...
        for metric in metrics:
            if metric in df.columns:
                ...
                sns.heatmap(
                    ...
                    cmap=(
                        "viridis"
                        if metric in ["throughput_rps", "quality_score"]
                        else "viridis_r"
                    ),
    ...
    def _calculate_pareto_frontier(
        self, df: pd.DataFrame, objectives: List[Tuple[str, str]]
    ) -> pd.Series:
...
<body>
    ...
    <ul>"""
        ...
        if "quantization" in df.columns:
            quant_performance = (
                df.groupby("quantization")["cost_per_1k_tokens"].mean().sort_values()
            )
            best_quant = (
                quant_performance.index[0] if len(quant_performance) > 0 else "none"
            )
            html_report += (
                f"<li><strong>Best Quantization for Cost:</strong> {best_quant}</li>"
            )

        if "kv_cache_dtype" in df.columns:
            cache_performance = (
                df.groupby("kv_cache_dtype")["throughput_rps"]
                .mean()
                .sort_values(ascending=False)
            )
            best_cache = (
                cache_performance.index[0] if len(cache_performance) > 0 else "auto"
            )
            html_report += (
                f"<li><strong>Best KV Cache for Throughput:</strong> {best_cache}</li>"
            )

        # Cost savings calculation
        if "cost_per_1k_tokens" in df.columns:
            baseline_cost = df[df["quantization"] == "none"][
                "cost_per_1k_tokens"
            ].mean()
            best_cost = df["cost_per_1k_tokens"].min()
            if (
                not pd.isna(baseline_cost)
                and not pd.isna(best_cost)
                and baseline_cost > 0
            ):
        ...
        "sweep_configurations": [
            ...
            },
            {
                ...
                "decoding": {
                    "temperature": 0.0,
                    "top_p": 1.0,
                    "extra_openai": {"use_beam_search": True, "num_beams": 4},
                },
            },
            {
                "name": "gptq-balanced",
...
def main():
    ...
    parser.add_argument(
        "--config", default="quantization_sweep.yaml", help="Sweep configuration file"
    )
    parser.add_argument(
        "--output-dir",
        default="sweeps/quantization",
    ...
    if not Path(args.config).exists():
        ...
        print("Run with --create-config to generate default configuration")
        return 1
    ...
    if results:
        ...
        if not df_clean.empty and "cost_per_1k_tokens" in df_clean.columns:
            ...
            print(
                f"   💰 Cost: ${best_config.get('cost_per_1k_tokens', 0):.4f}/1K tokens"
            )
            print(f"   ⚡ P95: {best_config.get('p95_ms', 0):.1f}ms")
            print(
                f"   🔧 Config: {best_config.get('quantization', 'none')} + {best_config.get('kv_cache_dtype', 'auto')} KV"
            )
